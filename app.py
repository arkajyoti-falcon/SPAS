...
# Import process flow modules from Supportive_Functions
...
# ==================== COSTING SHEET COMPONENT EXTRACTION ====================
# Module: Extract components from costing workbook (Infeed, Induct, Main Loop, Output Chutes)
...
def _is_excel_error(v) -> bool:
    """Check if value is Excel error."""
    return isinstance(v, str) and v.strip().startswith("#")


def _to_float_money(v) -> float:
    """Convert value to float, handling NaN, errors, and strings."""
    if v is None:
        return 0.0
    try:
        if pd.isna(v):
...
def _to_int(v) -> int:
    """Convert value to int, handling NaN, errors, and strings."""
    if v is None:
        return 0
    try:
        if pd.isna(v):
...
def find_sheet_by_name_tolerant(wb: openpyxl.Workbook, desired: str) -> Optional[str]:
    """Find sheet by name with tolerance for whitespace/case."""
...
def choose_main_loop_type_from_visible(wb: openpyxl.Workbook) -> str:
    """Determine main loop type from visible sheets."""
...
def pick_costing_sheet_visible_only(wb: openpyxl.Workbook) -> str:
    """Pick visible costing sheet (Loop CBS or Linear CBS)."""
    vis = visible_sheetnames(wb)
    if not vis:
...
def read_section_kv(ws, section_header: str) -> Dict[str, Any]:
    """Read key-value pairs from section in worksheet."""
...
def extract_induct_counts_no_calc(kv: Dict[str, Any]) -> Dict[str, int]:
    """Extract induct component counts from key-value dict."""
    out = {
    ...
    for k, v in kv.items():
        ...
        elif "orientation conveyor" in kl:
...
def _looks_like_accessory(desc: str) -> bool:
    """Check if description is an accessory (not main component)."""
    d = " " + desc.lower().strip() + " "
    return any(k in d for k in EXCLUDE_CHUTE_KEYWORDS)


def _looks_like_chute(desc: str) -> bool:
    """Check if description looks like a chute."""
    d = desc.lower()
    if "chute" not in d:
        return False
...
def extract_output_chutes(uploaded_file, wb: openpyxl.Workbook) -> List[Dict[str, Any]]:
    """Extract output chutes from Destinations sheet."""
...
def detect_vds_presence_from_arm_sorter(wb: openpyxl.Workbook) -> str:
    """Detect VDS presence from Arm Sorter sheet."""
...
def extract_all_components_from_costing_sheet(costing_file) -> Dict[str, Any]:
    """
    Extract all components from costing sheet including:
    - Infeed System (with VDS)
    - Induct components
    - Main Loop type
    - Output Chutes
    
    Returns structured dict with all extracted components.
    """
    try:
        wb = openpyxl.load_workbook(costing_file, data_only=True)
        ...
        try:
            ...
            extracted["main_loop"]["type"] = main_loop_type
        except Exception as e:
            logger.warning(f"Could not determine main loop type: {e}")
            extracted["main_loop"]["type"] = "Loop CBS"
        
        # Get VDS status
        try:
            vds_status = detect_vds_presence_from_arm_sorter(wb)
            extracted["infeed_system"]["vds"] = vds_status
        except Exception as e:
        ...
        # Get output chutes
        try:
            if hasattr(costing_file, 'seek'):
                extracted["output_chutes"] = extract_output_chutes(costing_file, wb)
        except Exception as e:
            logger.warning(f"Could not extract output chutes: {e}")
        
        extracted["success"] = True
        return extracted
        
    except Exception as e:
        logger.error(f"Error extracting components from costing sheet: {e}")
        return {
            "success": False,
            "infeed_system": {"vds": "unknown"},
            "induct": [],
...
# ==================== DXF EXTRACTION & COMPONENT MAPPING (CBS Mapping + AI Prediction) ====================

# ---- Configuration (from ST_dxf_extract.py) ----
CBS_MAPPING_FILE = "CBS_Component_Mapping.xlsx"  # backend mapping file (Mechanical Name -> Actual Name + Section)
GROQ_BASE_URL = "https://api.groq.com/openai/v1"
GROQ_MODEL = "openai/gpt-oss-120b"

# Noise rules (aggressive) from ST_dxf_extract.py
...
def is_noise_block_name(name: str) -> bool:
    """Check if block name is noise (from ST_dxf_extract.py logic)."""
    if not name:
        return True
    n = str(name).strip()
    if not n:
...
def extract_base_fal_code(s: str) -> str:
    """Extract base FAL code (e.g. FAL_FS002V02) stripping suffixes like .1 or (text)."""
    if not s:
        return ""
...
def load_cbs_mapping():
    """
    Load CBS_Component_Mapping.xlsx and build lookup dictionaries.
    
    Returns:
        Tuple of (exact_lookup, norm_lookup, actual_names, section_lookup, mapping_pairs_df)
        - exact_lookup: {MECH_UPPER -> actual_name}
        - norm_lookup:  {NORMALIZED_KEY -> actual_name}
        - actual_names: sorted list of unique actual names (for AI allow-list)
        - section_lookup: {MECH_UPPER -> section, NORMALIZED_KEY -> section}
        - mapping_pairs_df: DataFrame of (Mechanical Name, Actual Name) for AI context
    """
    if not os.path.exists(CBS_MAPPING_FILE):
        raise FileNotFoundError(
            f"Backend mapping file not found: {CBS_MAPPING_FILE}. "
            f"Keep it in the same folder as app.py."
        )

    df = pd.read_excel(CBS_MAPPING_FILE)
    required_cols = ['Mechanical Name', 'Actual Name']
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        raise ValueError(f"CBS mapping file missing columns: {missing}")
    ...
    for _, r in df.iterrows():
        ...
        section = str(r.get("Section") or "Others").strip() if has_section else "Others"
        if not mech or not actual:
            continue

        mech_up = mech.upper()
        exact_lookup.setdefault(mech_up, actual)
        section_lookup.setdefault(mech_up, section)

        norm = normalize_mech_key(mech_up)
        if norm:
            norm_lookup.setdefault(norm, actual)
            section_lookup.setdefault(norm, section)
    ...
    return exact_lookup, norm_lookup, actual_names, section_lookup, mapping_pairs


def _groq_api_key():
    """Get Groq API key from st.secrets or environment."""
    try:
...
def _build_mapping_context_text(mapping_pairs_df: pd.DataFrame, max_chars: int = 60000) -> str:
    """Build text context of mapping pairs for AI prompt."""
    lines = []
    used = 0
...
def _build_ai_system_prompt() -> str:
    """Build system prompt for Groq AI prediction of unmapped components."""
    return (
...
def groq_predict_actual_names(unmapped_components: List[str], actual_names: List[str], mapping_context: str) -> Dict:
    """
    Call Groq API to predict Actual Names for unmapped DXF components.
    
    Args:
        unmapped_components: list of mechanical name strings
        actual_names: allowed actual names list
        mapping_context: text of reference mapping pairs
        
    Returns:
        dict: {MECH_UPPER -> {"actual_name": str, "confidence": float}}
    """
    key = _groq_api_key()
    if not key:
...
def _map_single_component(component_name: str, exact_lookup: Dict, norm_lookup: Dict):
    """
    Map a single component name to its Actual Name using exact & normalized lookup.
    Returns actual_name or None if unmapped.
    """
...
def _get_section_for_component(component_name: str, section_lookup: Dict) -> str:
    """Get the section for a component from the section lookup, defaulting to 'Others'."""
    if not component_name:
        return "Others"
    comp_up = str(component_name).strip().upper()
    if comp_up in section_lookup:
        return section_lookup[comp_up]
    norm = normalize_mech_key(comp_up)
    if norm and norm in section_lookup:
        return section_lookup[norm]
    return "Others"


def read_dxf_inserts(dxf_path: str) -> tuple:
    """
    Read DXF file, extract INSERT entities from modelspace, filter noise.
    
    Returns:
        Tuple of (inserts_list, stats_dict)
        - inserts_list: [{component_name, x, y, z}, ...]
        - stats_dict: {total_inserts, removed_noise, kept}
    """
    try:
        doc, _aud = ezdxf.recover.readfile(str(dxf_path))
    except Exception:
...
def extract_and_map_dxf_components(dxf_path: str) -> tuple:
    """
    Full DXF extraction and mapping pipeline with Excel integration and AI fallback.
    
    WORKFLOW:
    1. Extract INSERT blocks from DXF (with noise removal)
    2. Map components via CBS_Component_Mapping.xlsx
    3. Identify UNMAPPED components → Add ALL to Excel with empty "Actual Name" for manual mapping later
    4. Send ALL UNMAPPED components to Groq AI for prediction
    5. Apply confidence threshold (>0.5) - only accept AI predictions with confidence > 50%
    6. Build final component list with status:
       - MAPPED: Found in Excel
       - AI_MAPPED: AI predicted with confidence > 0.5
       - UNMAPPED: Still unknown (AI failed or confidence ≤ 0.5)
    
    Returns:
        Tuple of (mapped_components, detailed_stats, ai_prediction_count)
        - mapped_components: List[Dict] with keys:
            * dxf_name: original component name from DXF
            * proposal_name: mapped actual name (or [UNMAPPED] prefix if still unknown)
            * count: number of occurrences
            * section: assigned section
            * status: 'MAPPED' | 'AI_MAPPED' | 'UNMAPPED'
            * ai_generated: bool, True if AI predicted
            * ai_confidence: float, AI confidence score (0-1) or None
        - detailed_stats: dict with extraction statistics including:
            * total_inserts, removed_noise, kept
            * initial_unmapped_count: count before AI prediction
            * added_to_excel_count: components added to Excel
            * ai_prediction_count: successful AI predictions (confidence > 0.5)
        - ai_prediction_count: number of components successfully predicted by AI (confidence > 0.5)
    """
    # Canonical section names
    CANONICAL_SECTIONS = {
        "infeed system": "Infeed System",
        "induct system": "Induct System",
        "irregular's takeout": "Irregular's Takeout",
        "irregulars takeout": "Irregular's Takeout",
        "irregular takeout": "Irregular's Takeout",
        "loop cbs": "Loop CBS",
        "output chutes": "Output Chutes",
        "others": "Others",
        "unmapped": "Unmapped",
    }
    
    def _normalize_section(raw_section: str) -> str:
        if not raw_section:
            return "Others"
        lookup = raw_section.strip().lower()
        return CANONICAL_SECTIONS.get(lookup, raw_section.strip())
    
    # Step 1: Load CBS mapping
    exact_lookup, norm_lookup, actual_names, section_lookup, mapping_pairs = load_cbs_mapping()
    
    # Step 2: Extract DXF inserts with noise removal
    inserts, stats = read_dxf_inserts(dxf_path)
    
    # Step 3: Aggregate per component name (with count + avg position)
    ...
    # Step 4: Map each unique component name via Excel (exact + normalized)
    mapped_names = []   # names that found a match in Excel
    unmapped_names = [] # names that need AI prediction
    component_data = {} # mech_name -> {count, x, y, z}
    ...
    for mech, a in agg.items():
        if a["pos_n"] > 0:
            ...
            ax = ay = az = None
        component_data[mech] = {"count": a["count"], "x": ax, "y": ay, "z": az}
        
        actual = _map_single_component(mech, exact_lookup, norm_lookup)
        if actual is not None:
            mapped_names.append(mech)
        else:
            unmapped_names.append(mech)
    
    # Step 4.5: Add UNMAPPED components to Excel sheet BEFORE sending to AI
    # This allows users to manually map them later in the Component Mapping page
    added_to_excel_count = 0
    if len(unmapped_names) > 0:
        try:
            excel_path = Path(__file__).parent / "CBS_Component_Mapping_WITH_DESCRIPTION.xlsx"
            if excel_path.exists():
                # Load existing mapping
                df_mapping = pd.read_excel(excel_path, sheet_name="CBS Component Map")
                
                # Get existing mechanical names (normalized for comparison)
                existing_mech_names = set(
                    str(name).strip().lower() 
                    for name in df_mapping.get("Mechanical Name", []).fillna("").astype(str)
                )
                
                # Filter out unmapped components that already exist
                new_unmapped = [
                    name for name in unmapped_names
                    if name.lower() not in existing_mech_names
                ]
                
                if new_unmapped:
                    # Create new rows for unmapped components
                    new_rows = pd.DataFrame({
                        "Mechanical Name": new_unmapped,
                        "Actual Name": [""] * len(new_unmapped),
                        "Section": ["Unmapped"] * len(new_unmapped),
                        "description": ["Auto-added from DXF (pending mapping)"] * len(new_unmapped)
                    })
                    
                    # Append to existing data
                    df_mapping = pd.concat([df_mapping, new_rows], ignore_index=True)
                    
                    # Remove duplicates
                    df_mapping = df_mapping.drop_duplicates(subset=["Mechanical Name"], keep="first")
                    
                    # Write back to Excel
                    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
                        df_mapping.to_excel(writer, sheet_name="CBS Component Map", index=False)
                    
                    added_to_excel_count = len(new_unmapped)
                    logger.info(f"Added {added_to_excel_count} unmapped components to Excel before AI prediction")
        except Exception as e:
            logger.warning(f"Failed to add unmapped components to Excel: {e}")
    
    # Track total unmapped count before AI (for stats)
    initial_unmapped_count = len(unmapped_names)
    
    # Step 5: AI prediction for unmapped components (with confidence threshold > 0.5)
    ai_predictions = {}
    ...
    if len(unmapped_names) > 0:
        try:
            mapping_context = _build_mapping_context_text(mapping_pairs, max_chars=60000)
            ai_predictions = groq_predict_actual_names(unmapped_names, actual_names, mapping_context)
            # Count only predictions with confidence > 0.5
            ai_prediction_count = sum(
                1 for v in ai_predictions.values() 
                if v.get("actual_name") != "UNKNOWN" and float(v.get("confidence", 0.0) or 0.0) > CONFIDENCE_THRESHOLD
            )
        except Exception as ex:
            logger.warning(f"Groq AI mapping failed: {ex}")
            ai_predictions = {}
    
    # Step 6: Build final mapped components list
    result = []
    for mech in list(agg.keys()):
        data = component_data[mech]
        mech_up = mech.upper()
        
        # Try Excel mapping first
        actual = _map_single_component(mech, exact_lookup, norm_lookup)
        ai_generated = False
        ai_confidence = None
        ...
        if actual is not None:
            # Mapped via Excel
            section = _normalize_section(_get_section_for_component(mech, section_lookup))
            status = 'MAPPED'
        else:
            # Try AI prediction (with confidence threshold > 0.5)
            pred = ai_predictions.get(mech_up, {"actual_name": "UNKNOWN", "confidence": 0.0})
            pred_confidence = float(pred.get("confidence", 0.0) or 0.0)
            
            if pred.get("actual_name") and pred["actual_name"] != "UNKNOWN" and pred_confidence > 0.5:
                actual = pred["actual_name"]
                ai_generated = True
                ai_confidence = pred_confidence
                # Try to find section for AI-predicted actual name by reverse lookup
                section = "Others"
                for mk, an in exact_lookup.items():
                    if an == actual and mk in section_lookup:
                        section = _normalize_section(section_lookup[mk])
                        break
                status = 'AI_MAPPED'
            else:
                actual = f"[UNMAPPED] {mech}"
                section = 'Unmapped'
                status = 'UNMAPPED'
        
        result.append({
            'dxf_name': mech,
            'proposal_name': actual,
            'count': data["count"],
            'section': section,
            'status': status,
    ...
    detailed_stats = {
        **stats,
        'initial_unmapped_count': initial_unmapped_count,
        'added_to_excel_count': added_to_excel_count,
        'ai_prediction_count': ai_prediction_count
    }
    
    return result, detailed_stats, ai_prediction_count


def _post_process_section_assignments(components: List[Dict]) -> List[Dict]:
    """
    Smart post-processing: reclassify components whose names indicate they belong
    to a different section than what the Excel mapping assigned.
    
    Rules:
    - Components with 'irregular' in the name → "Irregular's Takeout"
      (regardless of what the Excel says)
    """
    IRREGULAR_KEYWORDS = ["irregular"]
    
    for comp in components:
        name_lower = comp.get('proposal_name', '').lower()
        current_section = comp.get('section', '')
        
        # Rule: Irregular components → Irregular's Takeout
        if any(kw in name_lower for kw in IRREGULAR_KEYWORDS):
            if current_section != "Irregular's Takeout":
                comp['section'] = "Irregular's Takeout"
...
def _merge_duplicate_components(components: List[Dict]) -> List[Dict]:
    """
    Merge components that have the same proposal_name within the same section.
    
    When multiple DXF blocks map to the same proposal name (e.g., 'FeedLineTransferPlate'
    and 'FeedLineTransferPlate with 600 mm' both map to 'Feedline transfer'), this function
    consolidates them into a single entry with summed counts.
    
    Tracks zone_count (how many original DXF blocks contributed) and preserves all
    original dxf_names for traceability.
    """
    # Key: (proposal_name, section) → merged component
    merge_map = {}
    
    for comp in components:
        key = (comp.get('proposal_name', '').strip(), comp.get('section', 'Others'))
        
        if key in merge_map:
            existing = merge_map[key]
            existing['count'] += comp['count']
            existing['zone_count'] += 1
            # Track all contributing DXF names
            existing_dxf = existing.get('dxf_name', '')
            new_dxf = comp.get('dxf_name', '')
            if new_dxf and new_dxf not in existing_dxf:
                existing['dxf_name'] = f"{existing_dxf}, {new_dxf}" if existing_dxf else new_dxf
        else:
            merged = comp.copy()
            merged['zone_count'] = 1
            merge_map[key] = merged
    
    return list(merge_map.values())


def _group_components_by_section(components: List[Dict]) -> Dict[str, List[Dict]]:
    """Group components by their section (case-insensitive matching to canonical names)."""
    grouped = {}
    section_order = ["Infeed System", "Induct System", "Irregular's Takeout", "Loop CBS", "Output Chutes", "Others"]
    
    # Build a case-insensitive lookup for canonical section names
    canonical_lookup = {s.lower(): s for s in section_order}
    
    # Initialize sections in order
    for section in section_order:
        grouped[section] = []
    
    # Group components with case-insensitive section matching
    for comp in components:
        raw_section = comp.get('section', 'Others')
        # Normalize to canonical name
        section = canonical_lookup.get(raw_section.lower(), raw_section)
        if section not in grouped:
            grouped[section] = []
        grouped[section].append(comp)
    
    return grouped


def classify_induct_components(induct_components: List[Dict]):
    """
    Classify induct system components into Parcel Induct and Bag Induct sub-groups.
    
    Detection logic:
    - Count how many separate "Loading Conveyor" entries exist in the induct section.
    - If there are 2+ Loading Conveyor entries (each with count 1), the system has BOTH
      parcel and bag inducts.
    - If only 1 or 0 Loading Conveyor entries, determine single type from other markers.
    
    Parcel Inducts: Feedline assemblies (each feedline bundles Loading + Buffer + Intelligent Merge)
    Bag Inducts: Individual conveyor modules (Loading Conveyor, 30° Belt Turn, Buffer Conveyor, Intelligent Merge)
    
    Returns:
        Tuple of (parcel_components, bag_components, detected_type)
        detected_type: "Parcel", "Bag", or "Both"
    """
    # Keywords that definitively identify Parcel Induct components
    parcel_keywords = ["feedline", "feedline assembly"]
    # Keywords that definitively identify Bag Induct components
    bag_keywords = ["belt turn", "30-degree", "30 deg", "bag induct"]
    
    # STEP 1: Count Loading Conveyor entries to detect Both
    loading_conveyor_entries = [
        comp for comp in induct_components
        if "loading conveyor" in comp.get('proposal_name', '').lower()
    ]
    has_duplicate_loading = len(loading_conveyor_entries) >= 2
    
    # Check for feedline presence (parcel marker)
    has_feedline = any(
        any(kw in comp.get('proposal_name', '').lower() for kw in parcel_keywords)
        for comp in induct_components
    )
    # Check for belt turn presence (bag marker)
    has_belt_turn = any(
        any(kw in comp.get('proposal_name', '').lower() for kw in bag_keywords)
        for comp in induct_components
    )
    
    # STEP 2: Determine detected type
    if has_duplicate_loading and has_feedline:
        # Two Loading Conveyors + Feedline => Both parcel and bag inducts
        detected_type = "Both"
    elif has_feedline and has_belt_turn:
        # Feedline + Belt Turn => Both
        detected_type = "Both"
    elif has_feedline:
        # Only feedline, no duplicate loading, no belt turn => Parcel only
        detected_type = "Parcel"
    elif has_belt_turn or has_duplicate_loading:
        # Belt turn present OR duplicate loading without feedline => Bag only
        detected_type = "Bag"
    else:
        # Fallback: single type, treat as Parcel if components exist
        detected_type = "Parcel" if induct_components else "Parcel"
    
    # STEP 3: Classify components into parcel/bag based on detected type
    parcel_components = []
    bag_components = []
    
    if detected_type == "Both":
        # Feedline => Parcel. Belt turn => Bag.
        # For Loading Conveyor: first one goes to Parcel, rest go to Bag.
        # Other ambiguous items (Buffer Conveyor, Intelligent Merge) go to Bag
        #   because Parcel feedlines already bundle them internally.
        loading_assigned_to_parcel = False
        
        for comp in induct_components:
            name_lower = comp.get('proposal_name', '').lower()
            comp_copy = comp.copy()
            
            if any(kw in name_lower for kw in parcel_keywords):
                # Feedline => Parcel
                comp_copy['induct_type'] = 'parcel'
                parcel_components.append(comp_copy)
            elif any(kw in name_lower for kw in bag_keywords):
                # Belt turn => Bag
                comp_copy['induct_type'] = 'bag'
                bag_components.append(comp_copy)
            elif "loading conveyor" in name_lower:
                if not loading_assigned_to_parcel:
                    # First Loading Conveyor => Parcel (the feedline's loading module)
                    comp_copy['induct_type'] = 'parcel'
                    parcel_components.append(comp_copy)
                    loading_assigned_to_parcel = True
                else:
                    # Second+ Loading Conveyor => Bag
                    comp_copy['induct_type'] = 'bag'
                    bag_components.append(comp_copy)
            else:
                # Ambiguous (Buffer Conveyor, Intelligent Merge, etc.) => Bag
                # Because Parcel feedlines are a single DXF block that internally
                # contains these sub-modules, so standalone ones belong to Bag.
                comp_copy['induct_type'] = 'bag'
                bag_components.append(comp_copy)
    
    elif detected_type == "Parcel":
        # Everything goes to Parcel
        for comp in induct_components:
            comp_copy = comp.copy()
            comp_copy['induct_type'] = 'parcel'
            parcel_components.append(comp_copy)
        ...
        # Everything goes to Bag
        for comp in induct_components:
            comp_copy = comp.copy()
            comp_copy['induct_type'] = 'bag'
            bag_components.append(comp_copy)
    
    return parcel_components, bag_components, detected_type


def _render_section_component_editor(section_name: str, components: List[Dict], section_key: str) -> List[Dict]:
    """
    Render editable component list for a section with proper session state persistence.
    
    Uses session state to track component edits across reruns.
    
    Returns:
        List[Dict]: Updated components list with any edits applied
    """
    # Initialize session state for this section if not exists
    session_key = f"components_{section_key}"
    if session_key not in st.session_state:
        st.session_state[session_key] = [comp.copy() for comp in components]
    
    # Get the current component list from session state
    section_components = st.session_state[session_key]
    
    st.markdown(f"<div style='margin: 12px 0;'></div>", unsafe_allow_html=True)
    
    # Column headers
    col1, col2, col3, col4 = st.columns([2.5, 1, 1, 0.3])
    with col1:
        st.markdown("<span style='font-weight: 600; font-size: 12px; color: #666;'>Component Name</span>", unsafe_allow_html=True)
    with col2:
        st.markdown("<span style='font-weight: 600; font-size: 12px; color: #666;'>Count</span>", unsafe_allow_html=True)
    with col3:
        st.markdown("<span style='font-weight: 600; font-size: 12px; color: #666;'>Section</span>", unsafe_allow_html=True)
    with col4:
        st.markdown("")
    
    st.markdown("<div style='border-top: 1px solid #e0e0e0; margin: 4px 0;'></div>", unsafe_allow_html=True)
    
    # Track which components to remove
    components_to_delete = []
    
    for idx, comp in enumerate(section_components):
        comp_key = f"{section_key}_comp_{idx}"
        ...
        with col1:
            ...
            comp_name = st.text_input(
                "Component name",
                value=comp.get('proposal_name', ''),
                key=f"{comp_key}_name",
                label_visibility="collapsed",
                placeholder="Component name"
            )
            comp['proposal_name'] = comp_name
        
        with col2:
            # Editable count
            comp_count = st.number_input(
                "Count",
                min_value=0,
                max_value=9999,
                value=int(comp.get('count', 1)),
                step=1,
            ...
            # Show zone indicator when component was merged from multiple DXF blocks
            zone_count = comp.get('zone_count', 1)
            if zone_count > 1:
                st.markdown(
                    f"<span style='font-size: 10px; color: #0066cc; background: #e8f0fe; "
                    f"padding: 2px 6px; border-radius: 8px;'>{zone_count} zones</span>",
                    unsafe_allow_html=True
                )
            
            # Show AI badge if component was predicted by AI
            if comp.get('ai_generated'):
                ai_conf = comp.get('ai_confidence', 0.0)
                conf_pct = f"{ai_conf * 100:.0f}%" if ai_conf else ""
                st.markdown(
                    f"<span style='font-size: 10px; color: #9333ea; background: #f3e8ff; "
                    f"padding: 2px 6px; border-radius: 8px;'>🤖 AI {conf_pct}</span>",
                    unsafe_allow_html=True
                )
        ...
        with col3:
            ...
            st.markdown(
                f"<div style='padding: 8px 12px; background-color: #f5f5f5; border-radius: 4px; font-size: 12px;'>"
                f"{comp.get('section', 'Others')}</div>",
                unsafe_allow_html=True
            )
        ...
        with col4:
            ...
            if st.button(
                ...
                components_to_delete.append(idx)
    
    # Remove deleted components in reverse order to preserve indices
    ...
    add_form_key = f"{section_key}_show_add_form"
    
    # Show add component button only if form is not visible
    if not st.session_state.get(add_form_key, False):
        if st.button(
            "+ Add Component",
            key=f"{section_key}_add",
            help="Add a new component to this section",
            use_container_width=True
        ):
            st.session_state[add_form_key] = True
            st.rerun()  # Immediately show the form without needing second click
    else:
        # Show add component form
        st.markdown("<div style='margin: 12px 0;'></div>", unsafe_allow_html=True)
        st.markdown(
            "<div style='padding: 12px; background-color: #f0f7ff; border-left: 4px solid #0066cc; border-radius: 4px;'>"
            "<span style='font-size: 12px; color: #0066cc;'><strong>➕ New Component</strong></span></div>",
            unsafe_allow_html=True
        )
        ...
        with btn_col1:
            if st.button(
                "✓ Add",
                key=f"{section_key}_confirm_add",
                use_container_width=True,
                type="primary"
            ):
                if new_name.strip():
                    new_comp = {
                        'proposal_name': new_name.strip(),
                        'count': new_count,
                        'section': section_name,
                        'status': 'MANUAL',
                        'dxf_name': ''
                    }
                    st.session_state[session_key].append(new_comp)
                    st.session_state[add_form_key] = False
                    st.rerun()  # Close form immediately
                else:
        ...
        with btn_col2:
            if st.button(
                "✗ Cancel",
                key=f"{section_key}_cancel_add",
                use_container_width=True
            ):
                st.session_state[add_form_key] = False
                st.rerun()  # Close form immediately
    
    # Return the persisted components from session state
    return st.session_state[session_key]


def _render_induct_system_section(induct_components: List[Dict]) -> None:
    """
    Render the Induct System section with auto-detected Parcel/Bag sub-grouping.
    
    Detection is automatic based on DXF data:
    - 2+ Loading Conveyor entries => Both parcel and bag inducts
    - Otherwise determined from feedline / belt turn markers
    
    No user selection needed — type is inferred from the DXF components.
    """
    # Classify components (only used for initial split; session state persists after first run)
    parcel_comps, bag_comps, detected_type = classify_induct_components(induct_components)
    
    # Initialize induct type in session state if not already set
    if "induct_type_selection" not in st.session_state:
        st.session_state.induct_type_selection = detected_type
    
    # Initialize sub-section session state
    ...
    if "components_induct_bag" not in st.session_state:
        st.session_state.components_induct_bag = [c.copy() for c in bag_comps]
    
    induct_type = st.session_state.induct_type_selection
    
    # Show auto-detected type as an info badge (not editable)
    type_colors = {"Parcel": ("#2c5282", "#ebf8ff", "#bee3f8"), "Bag": ("#c53030", "#fff5f5", "#feb2b2"), "Both": ("#2d3748", "#f0fff4", "#9ae6b4")}
    txt_color, bg_color, border_color = type_colors.get(induct_type, ("#2d3748", "#f7fafc", "#e2e8f0"))
    st.markdown(
        f"<div style='padding: 10px 14px; background: {bg_color}; border: 1px solid {border_color}; border-radius: 6px; margin-bottom: 12px;'>"
        f"<span style='font-size: 12px; color: {txt_color}; font-weight: 600;'>"
        f"Auto-detected Induct Type: <span style='display:inline-block; padding: 2px 10px; background: {txt_color}; color: #fff; border-radius: 12px; font-size: 11px; margin-left: 6px;'>{induct_type}</span>"
        f"</span>"
        f"</div>",
        unsafe_allow_html=True
    )
    ...
    if induct_type in ["Parcel", "Both"]:
        ...
        st.markdown(
            f"<div style='padding: 8px 12px; background: #ebf8ff; border-left: 3px solid #3182ce; border-radius: 4px; margin: 8px 0;'>"
            f"<strong style='font-size: 13px; color: #2c5282;'>📦 Parcel Inducts ({parcel_count})</strong>"
            f"<p style='font-size: 11px; color: #4a5568; margin: 2px 0 0 0;'>"
            f"Each feedline typically includes: Loading Conveyor + Buffer Conveyor + Intelligent Merge</p>"
            f"</div>",
            unsafe_allow_html=True
        )
        _render_section_component_editor("Induct System", parcel_comps, "induct_parcel")
    
    # Show Bag Inducts sub-section
    if induct_type in ["Bag", "Both"]:
        bag_count = len(st.session_state.get("components_induct_bag", []))
        st.markdown(
            f"<div style='padding: 8px 12px; background: #fff5f5; border-left: 3px solid #e53e3e; border-radius: 4px; margin: 8px 0;'>"
            f"<strong style='font-size: 13px; color: #c53030;'>🛍️ Bag Inducts ({bag_count})</strong>"
            f"<p style='font-size: 11px; color: #4a5568; margin: 2px 0 0 0;'>"
            f"Typically includes: Loading Conveyor + 30° Belt Turn + Buffer Conveyor + Intelligent Merge</p>"
            f"</div>",
            unsafe_allow_html=True
        )
        _render_section_component_editor("Induct System", bag_comps, "induct_bag")


def show_component_confirmation_dialog(
    mapped_components: List[Dict],
    dxf_filename: str,
    costing_sheet_components: Optional[Dict] = None
) -> Optional[List[Dict]]:
    """
    DEPRECATED - Use show_merged_components_dialog instead.
    This function is kept for backward compatibility.
    """
    pass


def show_merged_components_dialog(
    dxf_file,
    dxf_components: List[Dict],
    costing_sheet_data: Dict
) -> Optional[List[Dict]]:
    """
    Show merged components dialog combining:
    1. Components extracted from DXF (EZDXF + CBS Mapping + AI)
    2. Components extracted from Costing Sheet
    
    User can review and confirm the merged component list.
    
    Args:
        dxf_file: Uploaded DXF file object
        dxf_components: Components extracted from DXF
        costing_sheet_data: Components extracted from costing sheet
    """
    st.markdown(
        "<div style='margin-bottom: 16px;'>"
        "<h3 style='margin: 0 0 8px 0; color: #1a1a1a; font-size: 18px; font-weight: 600;'>🔀 Review & Merge Components</h3>"
        "<p style='margin: 0; color: #666; font-size: 13px;'>Below is the merged component list from both DXF and Costing Sheet sources.</p>"
        "</div>",
        unsafe_allow_html=True
    )
    ...
    with tab1:
        st.markdown("**Components extracted from the Costing Sheet:**")
        
        if costing_sheet_data and costing_sheet_data.get("success"):
            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f"**Main Loop:** `{costing_sheet_data.get('main_loop', {}).get('type', 'Unknown')}`")
                vds = costing_sheet_data.get("infeed_system", {}).get("vds", "unknown")
                vds_display = "✅ HAS" if vds == "has" else ("❌ NOT HAS" if vds == "not_has" else "❓ UNKNOWN")
                st.markdown(f"**VDS Status:** {vds_display}")
            
            # Induct Components
            induct_comps = costing_sheet_data.get("induct", [])
            if induct_comps:
                st.markdown("**Induct Components:**")
                for comp in induct_comps:
                    st.markdown(f"- {comp.get('component', 'Unknown')}: **{comp.get('count', 0)}** units")
            
            # Output Chutes
            output_chutes = costing_sheet_data.get("output_chutes", [])
            if output_chutes:
                st.markdown("**Output Chutes:**")
                for chute in output_chutes:
                    st.markdown(f"- {chute.get('type', 'Unknown')}: **{chute.get('count', 0)}** units")
        else:
            st.warning("No components found in costing sheet")
    
    # TAB 2: DXF Components
    with tab2:
        st.markdown("**Components extracted from DXF (EZDXF + CBS Mapping + AI Prediction):**")
        
        # Show extraction stats
        stats = st.session_state.get("dxf_extraction_stats", {})
        if stats:
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Total INSERTs", stats.get("total_inserts", 0))
            with col2:
                st.metric("Noise Removed", stats.get("removed_noise", 0))
            with col3:
                st.metric("Components Kept", stats.get("kept", 0))
            with col4:
        ...
        for section_name, components in grouped.items():
            if components:
                st.markdown(f"**{section_name}** ({len(components)} items)")
                for comp in components:
                    status_badge = "✅" if comp.get('status') == 'MAPPED' else ("🤖" if comp.get('status') == 'AI_MAPPED' else "⚠️")
                    st.markdown(
                        f"- {status_badge} **{comp.get('proposal_name', comp.get('dxf_name', 'Unknown'))}** "
                        f"(×{comp.get('count', 1)}) — {comp.get('section', 'Others')}"
                    )
    
    # TAB 3: Merged Final List (for confirmation)
    with tab3:
        st.markdown("**Final Merged Component List for Confirmation:**")
        
        # Store section_display_map for confirmation button
        section_display_map = {
            "Infeed System": "Infeed System",
            "Induct System": "Induct System",
            "Irregular's Takeout": "Irregular's Takeout",
        ...
        for display_name, real_section_name in section_display_map.items():
            ...
            if display_name == "Induct System":
                ...
                if induct_type_sel == "Parcel":
                    total_induct_count = parcel_count
                elif induct_type_sel == "Bag":
                    total_induct_count = bag_count
                else:
                    total_induct_count = parcel_count + bag_count
                
                with st.expander(
                    f"▾ {display_name} ({total_induct_count})",
                    expanded=True
                ):
                    _render_induct_system_section(section_components)
            else:
                component_count = len(st.session_state.get(f"components_{real_section_name.lower().replace(' ', '_')}", []))
                
                with st.expander(
                    f"{'▾' if display_name != 'Others' else '▾'} {display_name} ({component_count})",
                    expanded=(display_name not in ["Others"])
                ):
                    _render_section_component_editor(
                        real_section_name,
                        section_components,
                        real_section_name.lower().replace(" ", "_")
                    )
    ...
    with col1:
        if st.button(
            "✓ Confirm & Continue",
            key="confirm_merged_components",
            width="stretch",
            type="primary"
        ):
            # Collect all components from all sections in session state
            all_updated_components = []
            section_display_map = {
                "Infeed System": "Infeed System",
                "Induct System": "Induct System",
                "Irregular's Takeout": "Irregular's Takeout",
            ...
            for real_section_name in section_display_map.values():
                if real_section_name == "Induct System":
                    induct_type_sel = st.session_state.get("induct_type_selection", "Both")
                    if induct_type_sel in ["Parcel", "Both"]:
                        parcel_comps = st.session_state.get("components_induct_parcel", [])
                        for comp in parcel_comps:
                            comp['induct_type'] = 'parcel'
                        all_updated_components.extend(parcel_comps)
                    if induct_type_sel in ["Bag", "Both"]:
                        bag_comps = st.session_state.get("components_induct_bag", [])
                        for comp in bag_comps:
                            comp['induct_type'] = 'bag'
                        all_updated_components.extend(bag_comps)
                else:
                    session_key = f"components_{real_section_name.lower().replace(' ', '_')}"
                    section_components = st.session_state.get(session_key, [])
                    all_updated_components.extend(section_components)
            
            if len(all_updated_components) == 0:
                st.error("❌ Please add at least one component before confirming.")
                return
            else:
                # Store in session state for access after dialog closes
                st.session_state.confirmed_extracted_components = all_updated_components
                st.session_state.components_confirmed = True
                ...
                st.session_state.manual_components_confirmed = all_updated_components
                
                logger.info(f"User confirmed {len(all_updated_components)} merged components from DXF + Costing Sheet")
                st.rerun()
    
    with col2:
        if st.button(
            "✗ Cancel",
            key="cancel_merged_components",
            width="stretch"
        ):
            st.session_state.show_merged_components_dialog = False
            st.session_state.use_manual_components = False
            st.session_state.components_confirmed = False
            st.rerun()


def log_cmd_block(title: str, payload: object) -> None:
    """Print a structured block to stdout for CMD visibility."""
    print("\n" + "=" * 80)
    print(title)
    print("=" * 80)
...
Return ONLY valid JSON, no explanations."""
    ...
    log_cmd_block(
        "DXF INPUT TO AI (COMPONENT CONFIGURATION)",
        {
    ...
    try:
        ...
        log_cmd_block(
            "AI OUTPUT (COMPONENT CONFIGURATION)",
            {
...
def _build_components_summary(components_dict: Dict) -> str:
    ...
    section_order = {
        ...
        "induct": "2. Induct",
        "irregulars_takeout": "3. Irregular's Takeout",
        "sorter": "4. Loop CBS",
        "output_chutes": "5. Output Chutes"
    }
    
    for section_id in ["infeed_system", "induct", "irregulars_takeout", "sorter", "output_chutes"]:
...
def show_component_editing_dialog(dxf_file, costing_file=None) -> None:
    """
    Show a modal dialog for confirming extracted DXF components and optionally costing sheet components.
    
    COMPLETE WORKFLOW:
    1. Extract INSERT blocks from DXF with noise removal
    2. Map components via CBS_Component_Mapping.xlsx (exact + normalized FAL code matching)
    3. Calculate: Initial Unmapped = Total Unique - Mapped from Excel
    4. Add ALL UNMAPPED components to Excel sheet with empty "Actual Name" → Shows as "Pending" in Component Mapping page
    5. Send ALL UNMAPPED components to Groq AI for prediction
    6. Apply confidence threshold (>0.5) - only accept AI predictions with confidence > 50%
    7. Final categorization:
       - MAPPED: Found in Excel ✅
       - AI_MAPPED: AI predicted with confidence > 0.5 🤖
       - UNMAPPED: Still unknown (AI failed or confidence ≤ 0.5) ⚠️
    8. Show user comprehensive stats with component names breakdown
    9. Additionally, extract components from costing sheet if provided
    10. Upon confirmation, store components for proposal generation
    
    Example: 15 unique components total
             - 7 mapped from Excel
             - 8 unmapped → Add all 8 to Excel
             - Send 8 to AI → 5 predicted successfully (confidence > 0.5)
             - Final: 7 MAPPED + 5 AI_MAPPED + 3 UNMAPPED
    """
    
    # Check if user already confirmed - if so, close dialog
    if st.session_state.get("components_confirmed"):
        return
    
    # Initialize component extraction if not already done
    if "extracted_components_mapped" not in st.session_state:
        # Clear old section editor state when new dialog opens
        section_keys_to_clear = [k for k in st.session_state.keys() if k.startswith("components_")]
        for key in section_keys_to_clear:
            del st.session_state[key]
        
        # Also clear induct type selection
        if "induct_type_selection" in st.session_state:
            del st.session_state["induct_type_selection"]
        
        # Also clear add form states
        form_keys_to_clear = [k for k in st.session_state.keys() if k.endswith("_show_add_form")]
        for key in form_keys_to_clear:
            del st.session_state[key]
        
        try:
            # Get DXF file path
            dxf_path = dxf_file.name if hasattr(dxf_file, 'name') else str(dxf_file)
            
            # Full extraction pipeline: extract -> CBS map -> add unmapped to Excel -> AI predict -> result
            with st.spinner("Extracting components from DXF and mapping via CBS + AI..."):
                mapped_components, stats, ai_count = extract_and_map_dxf_components(dxf_path)
            
            # Post-process: auto-reclassify irregular components etc.
            mapped_components = _post_process_section_assignments(mapped_components)
            
            # Merge duplicate components (same proposal_name in same section)
            mapped_components = _merge_duplicate_components(mapped_components)
            
            st.session_state.extracted_components_mapped = mapped_components
            st.session_state.dxf_extraction_stats = stats
            st.session_state.dxf_ai_prediction_count = ai_count
            
        except Exception as e:
            st.error(f"❌ Error extracting components from DXF: {str(e)}")
            logger.exception("DXF extraction error in dialog")
            return
    
    # Show confirmation dialog
    mapped_components = st.session_state.get("extracted_components_mapped", [])
    
    if not mapped_components:
        st.warning("No components were extracted from the DXF file.")
        if st.button("Close", key="close_empty_components", type="secondary"):
            st.session_state.show_component_dialog = False
            return
        return
    
    # Show extraction stats with detailed breakdown and component names
    stats = st.session_state.get("dxf_extraction_stats", {})
    ai_count = st.session_state.get("dxf_ai_prediction_count", 0)
    
    # Calculate component status counts
    mapped_count = sum(1 for c in mapped_components if c.get('status') == 'MAPPED')
    ai_mapped_count = sum(1 for c in mapped_components if c.get('status') == 'AI_MAPPED')
    still_unmapped_count = sum(1 for c in mapped_components if c.get('status') == 'UNMAPPED')
    
    # Get component names for each category
    mapped_names = [c.get('dxf_name') for c in mapped_components if c.get('status') == 'MAPPED']
    ai_mapped_names = [
        f"{c.get('dxf_name')} → {c.get('proposal_name')} ({c.get('ai_confidence', 0):.0%})" 
        for c in mapped_components if c.get('status') == 'AI_MAPPED'
    ]
    still_unmapped_names = [c.get('dxf_name') for c in mapped_components if c.get('status') == 'UNMAPPED']
    
    # Get added to Excel count from stats
    added_to_excel = stats.get('added_to_excel_count', still_unmapped_count)
    initial_unmapped = stats.get('initial_unmapped_count', added_to_excel)
    
    if stats:
        # Main stats header
        st.markdown(f"""
        <div style='background: #f7fafc; padding: 1rem; border-radius: 8px; border-left: 4px solid #3182ce; margin-bottom: 1rem;'>
            <div style='font-size: 0.9rem; color: #2d3748;'>
                <strong>📊 DXF Extraction Summary</strong><br>
                <div style='margin-top: 0.5rem; display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;'>
                    <div>• Total INSERTs: <strong>{stats.get('total_inserts', 0)}</strong></div>
                    <div>• Noise Removed: <strong>{stats.get('removed_noise', 0)}</strong></div>
                    <div>• Components Kept: <strong>{stats.get('kept', 0)}</strong></div>
                    <div>• Unique Components: <strong>{len(mapped_components)}</strong></div>
                </div>
                <hr style='margin: 0.75rem 0; border: none; border-top: 1px solid #e2e8f0;'>
                <strong>🗺️ Mapping Status</strong><br>
                <div style='margin-top: 0.5rem; display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;'>
                    <div>✅ <span style='color: #22543d;'>Mapped from Excel: <strong>{mapped_count}</strong></span></div>
                    <div>🤖 <span style='color: #2c5282;'>AI Predicted (>{0.5:.0%}): <strong>{ai_mapped_count}</strong></span></div>
                    <div>⚠️ <span style='color: #c53030;'>Still Unmapped: <strong>{still_unmapped_count}</strong></span></div>
                    <div>📝 <span style='color: #744210;'>Added to Excel: <strong>{added_to_excel}</strong></span></div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        # Component names breakdown in expander
        with st.expander("📋 View Component Details", expanded=False):
            col1, col2 = st.columns(2)
            
            with col1:
                st.markdown("**✅ Mapped from Excel:**")
                if mapped_names:
                ...
                if ai_mapped_names:
                    for name in ai_mapped_names:
                        st.markdown(f"- {name}")
                else:
                    st.caption("None")
            
            with col2:
                st.markdown("**⚠️ Still Unmapped (Added to Excel as Pending):**")
                if still_unmapped_names:
                    for name in still_unmapped_names:
                        st.markdown(f"- `{name}`")
                    st.info(f"💡 These {len(still_unmapped_names)} components have been added to the Component Mapping page with 'Pending' status. You can manually map them there.")
                else:
                    st.success("✓ All components successfully mapped!")
    
    dxf_filename = dxf_file.name if hasattr(dxf_file, 'name') else "DXF File"
    
    # Extract components from costing sheet if provided
    costing_sheet_components = None
    if costing_file:
        try:
            with st.spinner("Extracting components from costing sheet..."):
                costing_sheet_components = extract_all_components_from_costing_sheet(costing_file)
        except Exception as e:
            logger.warning(f"Could not extract components from costing sheet: {e}")
            costing_sheet_components = {
                "success": False,
                "error": str(e),
                "infeed_system": {"vds": "unknown"},
                "induct": [],
    ...
    show_component_confirmation_dialog(mapped_components, dxf_filename, costing_sheet_components)


def _convert_mapped_components_to_context(mapped_components: List[Dict]) -> Dict:
    """
    Convert the mapped components list to a context format compatible
    with existing proposal generation code.
    
    Args:
        mapped_components: List of dicts from extract_and_map_dxf_components
        
    Returns:
        Dict: Context compatible with rest of the application
    """
    # Get the confirmed induct type
    induct_type = st.session_state.get("induct_type_selection", "Both")
    
    context = {
        "configured_components": {},
        "system_configuration": {
            "has_vds": "Unknown",
            "induction_type": induct_type,
            "sorter_type": "Unknown"
        },
        "sections_included": []
    }
    ...
    # Separate tracking for induct sub-types
    induct_parcel_components = []
    induct_bag_components = []
    
    for comp in mapped_components:
        section = comp.get('section', 'Other')
        if section == 'Unmapped' or comp.get('status') == 'IGNORE':
            continue
        
        comp_entry = {
            'name': comp['proposal_name'],
            'count': comp['count'],
            'unit': 'Nos',
            'dxf_name': comp.get('dxf_name', '')
        }
        
        if section == 'Induct System':
            # Sub-classify induct components
            comp_induct_type = comp.get('induct_type', 'bag')
            comp_entry['induct_type'] = comp_induct_type
            if comp_induct_type == 'parcel':
                induct_parcel_components.append(comp_entry)
            else:
                induct_bag_components.append(comp_entry)
            sections[section].append(comp_entry)
        else:
            sections[section].append(comp_entry)
    
    # Convert to context format
    section_mapping = {
        'Infeed System': 'infeed_system',
        'Induct System': 'induct',
        "Irregular's Takeout": 'irregulars_takeout',
        'Loop CBS': 'sorter',
        'Output Chutes': 'output_chutes'
    }
    
    for section_name, components_list in sections.items():
        section_id = section_mapping.get(section_name, section_name.lower().replace(' ', '_'))
        context["sections_included"].append(section_name)
        ...
        # Add induct sub-group info
        if section_name == "Induct System":
            section_data["induct_type"] = induct_type
            section_data["parcel_induct_components"] = induct_parcel_components
            section_data["bag_induct_components"] = induct_bag_components
        
        context["configured_components"][section_id] = section_data
    
    # Build summary string with induct sub-grouping
    summary_lines = []
    for section_id in ["infeed_system", "induct", "irregulars_takeout", "sorter", "output_chutes"]:
        if section_id in context["configured_components"]:
            section_data = context["configured_components"][section_id]
            section_name = section_data["section_name"]
            summary_lines.append(f"{section_name}")
            
            if section_id == "induct" and section_data.get("induct_type"):
                # Show induct sub-groups in summary
                induct_t = section_data["induct_type"]
                summary_lines.append(f"  [Induct Type: {induct_t}]")
                
                if induct_t in ["Parcel", "Both"] and section_data.get("parcel_induct_components"):
                    summary_lines.append(f"  Parcel Inducts:")
                    for comp in section_data["parcel_induct_components"]:
                        count_str = f"{comp['count']} {comp['unit']}" if comp['count'] > 0 else ""
                        summary_lines.append(f"    - {comp['name']}: {count_str}" if count_str else f"    - {comp['name']}")
                
                if induct_t in ["Bag", "Both"] and section_data.get("bag_induct_components"):
                    summary_lines.append(f"  Bag Inducts:")
                    for comp in section_data["bag_induct_components"]:
                        count_str = f"{comp['count']} {comp['unit']}" if comp['count'] > 0 else ""
                        summary_lines.append(f"    - {comp['name']}: {count_str}" if count_str else f"    - {comp['name']}")
            else:
                for comp in section_data["components"]:
...
if st.session_state.page == "preview" and "generated_pdf_buffer" in st.session_state and st.session_state.generated_pdf_buffer is not None:
    ...
    with footer_left:
        if st.button("Back to Form", width="stretch"):
...
# ==================== CBS COMPONENT MAPPING PAGE ====================
    ...
    # Minimal, clean styling for CBS mapping page
    st.markdown("""
    <style>
    ...
    with back_col:
        if st.button("← Back", use_container_width=True, help="Return to proposal form"):
            st.session_state.page = "input"
            st.rerun()
    with title_col:
        st.markdown("<span style='font-size: 1.2rem; font-weight: 500; color: #2d3748;'>Component Mapping</span>", unsafe_allow_html=True)
    ...
    st.markdown(f"<div class='info-text'><strong>{len(df)}</strong> components configured</div>", unsafe_allow_html=True)
    st.markdown("<hr>", unsafe_allow_html=True)
    
    # Add status column - pending if Actual Name is empty, verified otherwise
    view = df.copy()
    view['_status'] = view['Component_name'].apply(
        lambda x: 'pending' if pd.isna(x) or str(x).strip() == '' else 'verified'
    )
    ...
    if search.strip():
        ...
        view = view[
            view["Component_Code"].str.lower().str.contains(s, na=False) |
            view["Component_name"].str.lower().str.contains(s, na=False) |
            view["Section"].str.lower().str.contains(s, na=False)
        ].copy()
    
    # Sort: pending first, then verified (within each group, sort by Component_name)
    status_order = {'pending': 0, 'verified': 1}
    view['_sort_order'] = view['_status'].map(status_order)
    view = view.sort_values(['_sort_order', 'Component_name']).drop(columns=['_sort_order'])
    ...
    if len(view) == 0:
        ...
        for _, r in view.reset_index().iterrows():
            ...
            if status == 'pending':
                status_badge = "<span style='background: #fed7d7; padding: 0.3rem 0.6rem; border-radius: 12px; font-size: 0.8rem; color: #c53030; font-weight: 600;'>⚠ Pending</span>"
            else:
                status_badge = "<span style='background: #c6f6d5; padding: 0.3rem 0.6rem; border-radius: 12px; font-size: 0.8rem; color: #22543d; font-weight: 600;'>✓ Verified</span>"
            
            cols = st.columns([2.5, 2.5, 1.5, 1.2, 3.5, 1.5, 0.5, 0.5])
            ...
            cols[3].markdown(f"<div style='padding: 0.5rem 0;'>{status_badge}</div>", unsafe_allow_html=True)
            cols[4].markdown(f"<div style='padding: 0.5rem 0; color: #718096; font-size: 0.9rem;'>{r['Description'][:100]}{'...' if len(r['Description']) > 100 else ''}</div>", unsafe_allow_html=True)
            ...
            if img_path:
                try:
                    cols[5].image(str(img_path), width=THUMB_WIDTH)
                except Exception as e:
                    cols[5].warning(f"Error loading image: {e}")
            else:
                cols[5].markdown("<div style='padding: 0.5rem 0; color: #cbd5e0; font-size: 0.85rem;'>No image</div>", unsafe_allow_html=True)
            
            if cols[6].button("✏️", key=f"edit_{idx}", help="Edit component"):
                st.session_state.edit_idx = idx
                edit_component_dialog()
            
            if cols[7].button("🗑️", key=f"del_{idx}", help="Delete component"):
...
# ==================== INPUT PAGE ====================
    ...
    st.markdown(
        """
        <style>
        /* Center and style main tabs */
        #main-tabs div[role=\"tablist\"] { justify-content: center; gap: 8px; }
        #main-tabs div[role=\"tablist\"] button[role=\"tab\"]:nth-child(2),
        #main-tabs div[role=\"tablist\"] button[role=\"tab\"]:nth-child(3),
        #main-tabs div[role=\"tablist\"] button[role=\"tab\"]:nth-child(4) {
            pointer-events: none;
        ...
        #main-tabs div[role=\"tablist\"] button[role=\"tab\"]:nth-child(2)::after,
        #main-tabs div[role=\"tablist\"] button[role=\"tab\"]:nth-child(3)::after,
        #main-tabs div[role=\"tablist\"] button[role=\"tab\"]:nth-child(4)::after {
            content: 'Coming soon';
            display: inline-block;
            margin-left: 8px;
    ...
    # ==================== CBS MAIN TAB ====================
    with main_cbs:
        # Create nested tabs for the 3 sections with CBS Component Mapping button inline
        tab_col1, tab_col2 = st.columns([5, 1])
        ...
        with tab_col2:
            ...
            if st.button("⚙️", use_container_width=False, help="Configure CBS component mappings", type="secondary", key="cbs_mapping_main"):
    ...
    with tab2:
        ...
        st.markdown("##### 📋 Component Extraction Workflow")
        st.info("Follow these steps: (1) Upload Costing Sheet → (2) Upload DXF → (3) Review & Confirm Merged Components")
        
        upload_col1, upload_col2 = st.columns(2)

        with upload_col1:
            # STEP 1: Upload Costing Sheet FIRST
            costing_file = st.file_uploader("**STEP 1:** Costing Sheet *", type=["xlsx", "xls"], key="costing_upload", help="Component pricing spreadsheet")
            
            costing_sheet_data = None
            if costing_file:
                # Extract components from costing sheet
                if "costing_sheet_components" not in st.session_state or st.session_state.get("last_costing_file") != costing_file.name:
                    try:
                        with st.spinner("Extracting components from costing sheet..."):
                            costing_sheet_data = extract_all_components_from_costing_sheet(costing_file)
                            st.session_state.costing_sheet_components = costing_sheet_data
                            st.session_state.last_costing_file = costing_file.name
                            if costing_sheet_data.get("success"):
                                st.success("✓ Costing sheet components extracted successfully")
                    except Exception as e:
                        st.error(f"❌ Error extracting from costing sheet: {str(e)}")
                        logger.exception("Costing sheet extraction error")
                else:
                    costing_sheet_data = st.session_state.get("costing_sheet_components")
        ...
        with upload_col2:
            # STEP 2: Upload DXF AFTER Costing Sheet
            if costing_file:
                dxf_layout_file = st.file_uploader("**STEP 2:** DXF Layout File *", type=["dxf"], key="dxf_upload", help="System layout in DXF format")
                
                if dxf_layout_file:
                    # Extract components from DXF using CBS mapping + AI prediction
                    if "extracted_components_mapped" not in st.session_state or st.session_state.get("last_dxf_name") != dxf_layout_file.name:
                        # Reset confirmation state for a new DXF
                        st.session_state.components_confirmed = False
                        try:
                            with st.spinner("Extracting components from DXF (EZDXF + CBS Mapping + AI)..."):
                                # Save uploaded DXF file to temporary path
                                with tempfile.NamedTemporaryFile(suffix=".dxf", delete=False) as tmp:
                                    tmp.write(dxf_layout_file.getbuffer())
                                    tmp_path = tmp.name
                                
                                # Full extraction pipeline: extract -> CBS map -> AI predict
                                mapped_components, stats, ai_count = extract_and_map_dxf_components(tmp_path)
                                
                                # Post-process section assignments
                                mapped_components = _post_process_section_assignments(mapped_components)
                                ...
                                # Log extraction info
                                log_cmd_block(
                                    "DXF COMPONENTS EXTRACTED (EZDXF + CBS Mapping + AI Prediction)",
                                    {
                                        "file_name": dxf_layout_file.name,
                                        "total_inserts": stats.get("total_inserts", 0),
                                        "noise_removed": stats.get("removed_noise", 0),
                                        "kept": stats.get("kept", 0),
                                        "ai_predicted": ai_count,
                                        "unique_components": len(mapped_components)
                                    },
                                )
                                ...
                                st.session_state.show_merged_components_dialog = True
                                st.success("✓ DXF components extracted successfully")
                                
                        except Exception as e:
                            st.error(f"❌ Error extracting components from DXF: {str(e)}")
                            logger.exception("DXF extraction error")
                    
                    # STEP 3: Show merged components dialog
                    if st.session_state.get("show_merged_components_dialog") and not st.session_state.get("components_confirmed"):
                        show_merged_components_dialog(
                            dxf_layout_file,
                            st.session_state.get("extracted_components_mapped", []),
                            st.session_state.get("costing_sheet_components", {})
                        )
                    
                    # Show confirmation status if components were confirmed
                    if st.session_state.get("components_confirmed"):
                        st.success(
                            f"✓ Components confirmed ({len(st.session_state.get('confirmed_extracted_components', []))} items ready for proposal generation)"
                        )
            else:
                st.warning("⚠️ Please upload the Costing Sheet first before uploading DXF")
                dxf_layout_file = None
    ...
    with tab3:
        ...
        generate_clicked = st.button("Generate Proposal", type="primary", width="stretch", help="Click to generate your complete proposal document")
...
def generate_system_description_from_sd_sys(dxf_path: Path, costing_file_upload, project_name: str,
                                           ...
                                           facts: Optional[ProposalFacts] = None,
                                           context: Optional[ProposalContext] = None,
                                           manual_components_context: Optional[Dict] = None) -> tuple[str, dict, dict, dict]:
    ...
    try:
        if context:
            ...
            from Supportive_Functions.proposal_context import build_proposal_context
        ...
        with st.spinner("Processing..."):
            try:
                ...
                if ENABLE_ALIAS_MAP and dxf_json:
                    try:
                        from Supportive_Functions.alias_map import analyze_and_update_aliases
                ...
                if ENABLE_CONSISTENCY_AUDIT:
                    try:
                        from Supportive_Functions.consistency_audit import audit_section_consistency
