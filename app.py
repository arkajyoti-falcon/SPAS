...
# ==================== DXF EXTRACTION & COMPONENT MAPPING (Using DXF2JSON Logic) ====================

# DXF extraction configuration (from DXF2JSON.py)
ANON_RE = re.compile(r"^\*?[UD]\d+$", re.IGNORECASE)
EXCLUDE_PREFIXES = [
    "FAL_BLK_ATT_",
    "ACAD_",
    "A$C",
...
def _is_noise_block_dxf(name: str) -> bool:
    """Check if block name is noise (from DXF2JSON.py logic)."""
    if not name:
        return True
    n = name.strip()
    if not n:
...
def _is_excluded_dxf(name: str) -> bool:
    """Check if block name is excluded (from DXF2JSON.py logic)."""
    up = name.strip().upper()
...
def _keep_block_dxf(name: str, include_noise: bool = False) -> bool:
    """Filter to keep only valid DXF component names (from DXF2JSON.py logic)."""
    if not name or not name.strip():
        return False
    if _is_excluded_dxf(name):
        return False
    if (not include_noise) and _is_noise_block_dxf(name):
        return False
    return True


def _build_block_children_dxf(doc) -> dict[str, Counter]:
    """Build parent->children mapping of DXF blocks (from DXF2JSON.py logic)."""
...
def _scale_counter_dxf(c: Counter, factor: int) -> Counter:
    """Scale counter values by factor (from DXF2JSON.py logic)."""
    if factor == 1:
        return c.copy()
    out = Counter()
    for k, v in c.items():
...
def _explode_block_dxf(
    block_name: str,
    block_children: dict[str, Counter],
    memo: dict[str, Counter],
    visiting: set[str],
) -> Counter:
    """Recursively expand nested blocks (from DXF2JSON.py logic)."""
    ...
    if not kids:
        ...
        out = Counter()
        out[block_name] += 1  # Include container
        
        for child, cnt in kids.items():
            child_expanded = _explode_block_dxf(child, block_children, memo, visiting)
            out += _scale_counter_dxf(child_expanded, cnt)
...
def extract_dxf_components_raw(dxf_path: Path) -> Counter:
    """
    Extract top-level component counts from DXF modelspace.
    
    Uses DXF2JSON.py logic to:
    1. Parse DXF file
    2. Extract INSERT blocks from modelspace
    3. Filter out noise/excluded blocks
    4. Return Counter of component names and counts
    
    Returns:
        Counter: {component_name: count, ...}
    """
...
def load_dxf_component_mapping(mapping_file: str = "DXF_MAP_PROPOSALNAME_CLEAN.xlsx") -> pd.DataFrame:
    """
    Load the DXF component to proposal name mapping from Excel.
    
    Expected columns:
    - 'DXF Component': Component name as it appears in DXF
    - 'Proposal Name': Friendly name to show in proposal
    - 'Section': Component category/section
    
    Returns:
        pd.DataFrame: Mapping dataframe with all rows
    """
    ...
    required_cols = ['DXF Component', 'Proposal Name', 'Section']
    missing_cols = [c for c in required_cols if c not in df.columns]
    if missing_cols:
        raise ValueError(f"Mapping file missing columns: {missing_cols}")
    
    return df


def map_dxf_components_to_proposal_names(
    dxf_components: Counter,
    mapping_df: pd.DataFrame
) -> List[Dict]:
    """
    Match extracted DXF components with proposal names from mapping.
    
    For each DXF component:
    - Look up in mapping_df
    - If found, use Proposal Name
    - If not found, mark as UNMAPPED
    - Return list of dicts with component info
    ...
    """
    # Canonical section names — map any case variation to the standard form
    CANONICAL_SECTIONS = {
    ...
    def _normalize_section(raw_section: str) -> str:
        """Normalize a section name from Excel to the canonical form."""
        if not raw_section:
            return "Others"
    ...
    for dxf_name, count in dxf_components.most_common():
        # Look up in mapping
        matches = mapping_df[mapping_df['DXF Component'].str.strip() == dxf_name.strip()]
        ...
        if len(matches) > 0:
            ...
            result.append({
                ...
                'count': count,
                'section': _normalize_section(str(row['Section'])),
                'status': 'MAPPED'
            })
        else:
            result.append({
                'dxf_name': dxf_name,
                'proposal_name': f"[UNMAPPED] {dxf_name}",
                'count': count,
                'section': 'Unmapped',
                'status': 'UNMAPPED'
            })
...
def show_component_editing_dialog(dxf_file) -> None:
    ...
    NEW WORKFLOW (Using DXF2JSON logic + Excel mapping):
    1. Extract raw components from uploaded DXF using DXF2JSON.py logic
    2. Load mapping from DXF_MAP_PROPOSALNAME_CLEAN.xlsx
    3. Match DXF component names to Proposal Names
    4. Show user the extracted components from 'Proposal Name' column
    5. Upon confirmation, store components for AI processing
    """
    ...
    if "extracted_components_mapped" not in st.session_state:
        ...
        try:
            # Extract raw components from DXF
            dxf_path = Path(dxf_file.name) if hasattr(dxf_file, 'name') else Path(str(dxf_file))
            raw_components = extract_dxf_components_raw(dxf_path)
            
            # Load mapping
            mapping_df = load_dxf_component_mapping()
            
            # Map to proposal names
            mapped_components = map_dxf_components_to_proposal_names(raw_components, mapping_df)
...
def _convert_mapped_components_to_context(mapped_components: List[Dict]) -> Dict:
    ...
    Args:
        mapped_components: List of dicts from map_dxf_components_to_proposal_names
...
# ==================== INPUT PAGE ====================
    ...
    with tab2:
        ...
        with upload_col1:
            ...
            if dxf_layout_file:
                # Extract components from DXF using DXF2JSON logic and show mapping dialog
                if "extracted_components_mapped" not in st.session_state or st.session_state.get("last_dxf_name") != dxf_layout_file.name:
                    ...
                    st.session_state.components_confirmed = False
                    # NEW WORKFLOW: Extract using DXF2JSON.py logic, then map to proposal names
                    try:
                        import tempfile
                        from pathlib import Path
                        ...
                        # STEP 1: Extract raw components using DXF2JSON logic
                        raw_components = extract_dxf_components_raw(Path(tmp_path))
                        
                        # STEP 2: Load mapping from Excel
                        mapping_df = load_dxf_component_mapping()
                        
                        # STEP 3: Map DXF component names to Proposal Names
                        mapped_components = map_dxf_components_to_proposal_names(raw_components, mapping_df)
                        
                        # STEP 4: Post-process section assignments (auto-reclassify irregular components etc.)
                        mapped_components = _post_process_section_assignments(mapped_components)
                        
                        # STEP 5: Merge duplicate components (same proposal_name in same section)
                        mapped_components = _merge_duplicate_components(mapped_components)
                        
                        # Log extraction info
                        log_cmd_block(
                            "DXF COMPONENTS EXTRACTED (Using DXF2JSON Logic)",
                            {
                                "file_name": dxf_layout_file.name,
                                "components_count": len(raw_components),
                                "raw_components": dict(raw_components),
                                "mapped_components_summary": [
                                    f"{c['proposal_name']} (x{c['count']}, {c['section']})"
                                    for c in mapped_components[:10]  # First 10 for brevity
                                ]
                            },
                        )
