...
def extract_and_map_dxf_components(dxf_path: str) -> tuple:
    """
    Full DXF extraction pipeline: extract -> map via CBS Excel -> AI predict unmapped -> return mapped components.
    
    This is the main entry point that replaces the old extract_dxf_components_raw + map_dxf_components_to_proposal_names flow.
    
    Returns:
        Tuple of (mapped_components, stats, ai_prediction_count)
        - mapped_components: List[Dict] with keys: dxf_name, proposal_name, count, section, status, ai_generated, ai_confidence
        - stats: dict with extraction statistics
        - ai_prediction_count: number of components predicted by AI
    """
    ...
    # Step 5: AI prediction for unmapped components
    ai_predictions = {}
    ...
    if len(unmapped_names) > 0:
        try:
            ...
            ai_prediction_count = sum(1 for v in ai_predictions.values() if v.get("actual_name") != "UNKNOWN")
    ...
    for mech in list(agg.keys()):
        ...
        if actual is not None:
            ...
            if pred.get("actual_name") and pred["actual_name"] != "UNKNOWN":
                ...
                ai_generated = True
                ai_confidence = float(pred.get("confidence", 0.0) or 0.0)
    ...
    return result, stats, ai_prediction_count
...
def show_component_confirmation_dialog(
    ...
    """
    Show section-based component editor dialog.
    
    Components are grouped by section (Infeed System, Induct System, Loop CBS, Output Chutes, Others).
    User can:
    - View components grouped by section
    - Edit component names
    - Edit component counts
    - Add new components to a section
    - Delete components
    - Confirm to proceed or cancel
    
    Returns:
        List[Dict]: Confirmed components with all edits applied, or None if cancelled
    """
    st.markdown(
        "<div style='margin-bottom: 16px;'>"
        "<h3 style='margin: 0 0 8px 0; color: #1a1a1a; font-size: 18px; font-weight: 600;'>✓ Components Extracted</h3>"
        f"<p style='margin: 0; color: #666; font-size: 13px;'><strong>DXF File:</strong> {dxf_filename}</p>"
        "</div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        "Components are organized by section below. You can edit names, adjust counts, add new components, or delete existing ones."
        "</p>",
        unsafe_allow_html=True
    )
    
    # Group components by section
    grouped_components = _group_components_by_section(mapped_components)
    
    # Count unmapped
    unmapped_count = sum(1 for c in mapped_components if c['status'] == 'UNMAPPED')
    ai_mapped_count = sum(1 for c in mapped_components if c.get('ai_generated'))
    
    # Show warning if there are unmapped components
    if unmapped_count > 0:
        st.warning(
            f"⚠️ {unmapped_count} component(s) could not be automatically mapped. "
            "These are marked as [UNMAPPED]. Please review and edit them."
        )
    
    # Show info if AI predicted some components
    if ai_mapped_count > 0:
        st.info(
            f"🤖 {ai_mapped_count} component(s) were predicted by AI. "
            "Please review their names carefully."
        )
    
    # Map of display names to section names (in preferred order)
    section_display_map = {
        "Infeed System": "Infeed System",
        "Induct System": "Induct System",
        "Irregular's Takeout": "Irregular's Takeout",
    ...
    for real_section_name in section_display_map.values():
        if real_section_name == "Induct System":
            # Special initialization: split Induct System into Parcel/Bag sub-groups
            ...
            if session_key not in st.session_state:
                # Get initial components for this section from grouped_components
    ...
    for display_name, real_section_name in section_display_map.items():
        ...
        if display_name == "Induct System":
            # Special rendering: Induct System with Parcel/Bag sub-groups
            induct_type_sel = st.session_state.get("induct_type_selection", "Both")
            parcel_count = len(st.session_state.get("components_induct_parcel", []))
            bag_count = len(st.session_state.get("components_induct_bag", []))
            # Show count based on selected induct type
            ...
            # Always show section, even if empty initially (user can add components)
            with st.expander(
    ...
    with col1:
        if st.button(
            "✓ Confirm & Continue",
            key="confirm_extracted_components",
            width="stretch",
            type="primary"
        ):
            ...
            for real_section_name in section_display_map.values():
                if real_section_name == "Induct System":
                    # Collect from Parcel and/or Bag sub-sections based on user selection
                    induct_type_sel = st.session_state.get("induct_type_selection", "Both")
            ...
            if len(all_updated_components) == 0:
                ...
                # Store induct type selection
                st.session_state.induct_type_confirmed = st.session_state.get("induct_type_selection", "Both")
                
                # Convert to context format for compatibility with existing code
                ...
                logger.info(f"User confirmed {len(all_updated_components)} components from DXF")
                st.rerun()  # Close dialog by triggering parent to re-evaluate show_component_dialog flag
    
    with col2:
        if st.button(
            "✗ Cancel",
            key="cancel_extracted_components",
            width="stretch"
        ):
            st.session_state.show_component_dialog = False
...
def show_component_editing_dialog(dxf_file) -> None:
    """
    Show a modal dialog for confirming extracted DXF components.
    
    NEW WORKFLOW (CBS Mapping + AI Prediction from ST_dxf_extract.py):
    1. Extract INSERT blocks from DXF with noise removal
    2. Map via CBS_Component_Mapping.xlsx (exact + normalized FAL code matching)
    3. Unmapped components are sent to Groq AI for prediction
    4. Show user the extracted components for editing
    5. Upon confirmation, store components for proposal generation
    """
    ...
    if "extracted_components_mapped" not in st.session_state:
        ...
        try:
            ...
            # Full extraction pipeline: extract -> CBS map -> AI predict -> result
    ...
    if stats:
        st.info(
            f"Modelspace INSERTs: {stats.get('total_inserts', 0)} | "
            f"Noise removed: {stats.get('removed_noise', 0)} | "
            f"Kept: {stats.get('kept', 0)} | "
            f"AI predicted: {ai_count}"
        )
    
    dxf_filename = dxf_file.name if hasattr(dxf_file, 'name') else "DXF File"
    
    # Show confirmation dialog directly
    show_component_confirmation_dialog(mapped_components, dxf_filename)
...
# ==================== INPUT PAGE ====================
    ...
    with tab2:
        ...
        with upload_col1:
            ...
            # When DXF is uploaded, automatically extract components and show confirmation dialog
            if dxf_layout_file:
                ...
                if "extracted_components_mapped" not in st.session_state or st.session_state.get("last_dxf_name") != dxf_layout_file.name:
                    ...
                    try:
                        ...
                        # Log extraction info
                        log_cmd_block(
                            "DXF COMPONENTS EXTRACTED (CBS Mapping + AI Prediction)",
                            {
                                ...
                                "mapped_components_summary": [
                                    f"{c['proposal_name']} (x{c['count']}, {c['section']}, {'AI' if c.get('ai_generated') else 'Excel'})"
                                    for c in mapped_components[:10]
                                ]
                            },
                        )
                        ...
                        # Clear old session state variables that are no longer used
                        if "dxf_components_extracted" in st.session_state:
                            del st.session_state.dxf_components_extracted
                        if "dxf_json_extracted" in st.session_state:
                            del st.session_state.dxf_json_extracted
                ...
                # Show confirmation dialog for component mapping
                if st.session_state.get("show_component_dialog") and not st.session_state.get("components_confirmed"):
                    show_component_editing_dialog(dxf_layout_file)
                
                # Show confirmation status if components were confirmed
                if st.session_state.get("components_confirmed"):
                    st.success(
                        f"✓ User confirmed the components ({len(st.session_state.get('confirmed_extracted_components', []))} items)"
                    )
